...
def game_field_init():
    ...
    #  cd_map = f'maps/map_4.txt'  # тестовая карта для разработчиков
    ...
    with open(cd_map, mode='rt') as map_file:
        ...
        for i, elem in enumerate(bricks):
            size -= elem.count('-')
    # randint(1, ...количество уровней) можно добавлять по шаблону map_... .txt
...
if __name__ == '__main__':
    ...
    while running:
        for event in pygame.event.get():
            ...
            #    if event.key == pygame.K_LEFT:  # включение режима разработчика
